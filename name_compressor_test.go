...
func TestBasicNameCompression(t *testing.T) {
	...
	nc.Record(5, []string{"foo", "com"})
	skip, offset := nc.Lookup([]string{"foo", "com"})
	if skip != 0 {
...
func TestReferenceLongerName(t *testing.T) {
	nc := dns.NewNameCompressor()
	nc.Record(5, []string{"foo", "bar", "com"})

	skip, offset := nc.Lookup([]string{"bar", "com"})
...
func TestReferenceShorterName(t *testing.T) {
	nc := dns.NewNameCompressor()
	nc.Record(5, []string{"bar", "com"})

	skip, offset := nc.Lookup([]string{"foo", "bar", "com"})
	if offset != 5 {
...
func TestReferenceNameWithSharedParent(t *testing.T) {
	nc := dns.NewNameCompressor()
	nc.Record(5, []string{"qux", "bar", "com"})
...
func TestLookupUnknownName(t *testing.T) {
	...
	_, offset := nc.Lookup([]string{"foo", "com"})
	if offset != 0 {
